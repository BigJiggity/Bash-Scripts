...
#  __promptline_wrapper "$(if [[ "${NVM_BIN}/node" == "$(which node || true)" ]]; then echo $NVM_BIN | grep -Eo "node/v([0-9]+\.?)+"; fi)" "$slice_prefix" "$slice_suffix" && { slice_prefix="$slice_joiner"; is_prompt_empty=0; }

  # section "b" header
  slice_prefix="${b_bg}${sep}${b_fg}${b_bg}${space}" slice_suffix="$space${b_sep_fg}" slice_joiner="${b_fg}${b_bg}${alt_sep}${space}" slice_empty_prefix="${b_fg}${b_bg}${space}"
  [ $is_prompt_empty -eq 1 ] && slice_prefix="$slice_empty_prefix"
  # section "b" slices
  __promptline_wrapper "$(hostname)" "$slice_prefix" "$slice_suffix" && { slice_prefix="$slice_joiner"; is_prompt_empty=0; }
...
function __promptline {
  ...
  if [[ -n ${ZSH_VERSION-} ]]; then
    ...
    #echo "${PROMPT}"
    #local left="$(__promptline_ps1)"
    #local pad="$(($COLUMNS-${#left}))"
    #PROMPT="$left${(l:$pad:: :)right}
    PROMPT="$(__promptline_ps1)
"
    local datestamp="$(date +'%a %b %d %k:%M')"
    local lineup=$'\e[1A'
    local linedown=$'\e[1B'
    RPROMPT="%{${lineup}%}${datestamp}%{${linedown}%}"
  elif [[ -n ${FISH_VERSION-} ]]; then
    if [[ -n "$1" ]]; then
